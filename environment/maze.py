...
class Maze:
    ...
    def __init__(self, maze, start_cell=(0, 5), exit_cell=[(7, 7), (2, 2), (4, 4)]):
        ...
        if len(self.__exit_cell) == 2:
            self.empty.remove(self.__exit_cell)
        else:
            for exits in self.__exit_cell:
                self.empty.remove(exits)
        ...
        if len(self.__exit_cell) == 2:
            ...
            for exits in self.__exit_cell:
                if exits not in self.cells:
                    raise Exception("Error: exit cell at {} is not inside maze".format(exits))
                if self.maze[exits[::-1]] == Cell.OCCUPIED:
                    raise Exception("Error: exit cell at {} is not free".format(exits))
    ...
    def reset(self, start_cell=(0, 0)):
        ...
        if self.__render in (Render.TRAINING, Render.MOVES):
            ...
            self.__ax1.plot(*self.__current_cell, "rs", markersize=3)  # start is a big red square
            self.__ax1.text(*self.__current_cell, "Start", ha="center", va="center", color="white")
            for exits in self.__exit_cell:
                self.__ax1.plot(*exits, "gs", markersize=3)  # exit is a big green square
                self.__ax1.text(*exits, "Exit", ha="center", va="center", color="white")

            #self.__ax1.plot(self.gas[self.time, :, :])
    ...
    def step(self, action, episodes=0):
    ...
    def __execute(self, action):
        ...
        if not possible_actions:
            ...
            if self.__render != Render.NOTHING and self.episodes >= 150:
                self.__draw()
            #TODO: add reward for gas
            if self.__current_cell in self.__exit_cell:
            ...
            #GAS
            # col, row = self.__current_cell
            # if self.time < self.gas.shape[0]:
            #     #reward -= self.gas[self.time, row, col]
            #     pass
    ...
    def render_q(self, model):
        ...
        if self.__render == Render.TRAINING:
            ...
            for exits in self.__exit_cell:
                self.__ax2.plot(*exits, "gs", markersize=3)  # exit is a big green square
                self.__ax2.text(*exits, "Exit", ha="center", va="center", color="white")
